...
const CarouselFeature: React.FC = () => {
   ...
   const getCardClass = useCallback(
      (target: string, index: number) => {
         ...
         if (index === (currentCard - 1 + cardData.length) % cardData.length)
   ...
   const resetCardTransforms = (e: Event) => {
      ...
      const currentInfoEl = refs.cardInfosContainer.current?.querySelector(
         '.current--info',
      ) as HTMLElement;
   ...
   const changeInfo = useCallback(
      (direction: string) => {
         ...
         const previousInfoEl = refs.cardInfosContainer.current?.querySelector(
            '.previous--info',
         ) as HTMLElement;
         const nextInfoEl = refs.cardInfosContainer.current?.querySelector(
            '.next--info',
         ) as HTMLElement;
   ...
   const swapCards = useCallback(
      (direction: string) => {
         const currentCardEl = refs.cardsContainer.current?.querySelector(
            '.current--card',
         ) as HTMLElement;
         const previousCardEl = refs.cardsContainer.current?.querySelector(
            '.previous--card',
         ) as HTMLElement;
         const nextCardEl = refs.cardsContainer.current?.querySelector(
            '.next--card',
         ) as HTMLElement;

         const currentBgImageEl = refs.appBgContainer.current?.querySelector(
            '.current--image',
         ) as HTMLElement;
         const previousBgImageEl = refs.appBgContainer.current?.querySelector(
            '.previous--image',
         ) as HTMLElement;
         const nextBgImageEl = refs.appBgContainer.current?.querySelector(
            '.next--image',
         ) as HTMLElement;
         ...
         <div className="app">
            <div className="landing--header">
               ...
               <p className="landing--p">
                  Festivals in the Philippines you must experience and find the greatness of its
                  culture. Share our similarities, celebrate our differences.
               </p>
            </div>
               ...
               <div className="info__wrapper" ref={refs.cardInfosContainer}>
                  {cardData.map((card, index) => (
                     <CarouselCardInfo
                        ...
                        className={getCardClass('info', index)}
                     />
                  ))}
               </div>
            </div>
