...
const FestivalGallery: React.FC<FestivalGalleryProps> = ({ festivals }) => {
    ...
    const renderCategoryButtons = () => {
        return categories.map((category, index) => (
            <button key={category} className={`filter-btn ${category === 'all' ? 'active' : ''}`} type="button" data-id={category} onClick={() => handleCategoryClick(category)} ref={btnsRef.current[index] as LegacyRef<HTMLButtonElement>}>
                {category}
            </button>
        ));
    };
        ...
        <>
                ...
                <div className={`search-festival ${searchActive ? 'search-active' : ''}`} ref={searchFestRef}>
                    ...
                    {searchActive && (
                        <div className="input-search">
                            <input type="text" placeholder="Search for a Festival..." id="search-inp-fest" value={searchKeyword} onChange={handleSearchInputChange} ref={searchBarRef} />
                        </div>
                    )}
            ...
            {filteredContent.length === 0 ? (
                <div id="search-result-empty-state" className="search-empty-state" ref={searchEmptyElRef}>
                    <Image className="search-empty-icon" alt="Search no result illustration" src="/images/search-empty-icon.svg" width={50} height={50} />
                ...
                <main className="fest-gallery-main">
                    ...
                    {filteredContent.map((festival) => (
                        <FestivalCardtem key={festival.id} id={festival.id.toString()} img={festival.img} title={festival.title} description={festival.paragraph1} />
                    ))}
                </main>
            )}
        </>
